...
const termSchema = yup.object().shape({
	names: yup.array().of(yup.string().required()).required().min(1, "Termin musi mieć co najmniej 1 polskie tłumaczenie.").transform((value) => 
		value.map((name) => name.trim().replace(/\s+/g, ' ')).filter((name) => name.length > 0)
	),
	definition: yup.string().required("Termin musi mieć definicję."),
	...
	})).required().min(1, "Termin musi mieć co najmniej 1 angielskie tłumaczenie.").transform((value) => {
